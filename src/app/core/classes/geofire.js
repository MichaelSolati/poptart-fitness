...
var GeoFire = (function () {
  ...
  GeoFire.distance = function (location1, location2) {
    ...
    if (!Array.isArray(location1)) {
      location1 = [location1.coordinates.lat, location1.coordinates.lng];
    }
    if (!Array.isArray(location2)) {
      location2 = [location2.coordinates.lat, location2.coordinates.lng];
    }
  ...
  var validateLocation = function (location) {
    var error;
    if ((location.coordinates && !isNaN(Number(location.coordinates.lat)) && !isNaN(Number(location.coordinates.lng)))) {
      return;
